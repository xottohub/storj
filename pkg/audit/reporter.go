...
func NewReporter(overlay *overlay.Cache, containmentDB DB, maxRetries int) *Reporter {
	return &Reporter{overlay: overlay, containmentDB: containmentDB, maxRetries: maxRetries}
}
...
func (reporter *Reporter) recordPendingAudits(ctx context.Context, pendingAudits []*PendingAudit) (failed []*PendingAudit, err error) {
	for _, pendingAudit := range pendingAudits {
		err = reporter.containmentDB.IncrementPending(ctx, pendingAudit)
		if err != nil {
